...
const DesignSection: React.FC = () => {
  return (
    <section className="w-full bg-[#FAFAFA] text-black py-32 px-6 md:px-12 lg:px-16 overflow-hidden">
      
      {/* Header Section */}
      <div className="flex flex-col lg:flex-row items-end justify-between mb-24 gap-8">
        <div className="max-w-3xl relative">
          <div className="absolute -left-6 top-1 w-1 h-12 bg-black/10 hidden lg:block"></div>
        ...
        <div className="flex flex-col items-start lg:items-end text-left lg:text-right max-w-sm gap-8">
          ...
          <button className="group flex items-center gap-2 text-black font-medium border-b border-black pb-1 hover:text-gray-600 hover:border-gray-400 transition-all text-sm tracking-wide">
            INICIAR PROJETO
        ...
        <div className="md:col-span-12 lg:col-span-4 bg-white p-10 rounded-[2rem] flex flex-col justify-between min-h-[400px] shadow-sm hover:shadow-xl transition-all duration-700 ease-out hover:-translate-y-1 border border-gray-100/50 group">
          <div>
            <div className="flex space-x-1 mb-8 opacity-80 group-hover:opacity-100 transition-opacity">
              {[1, 2, 3, 4, 5].map((star) => (
                <Star key={star} size={14} className="text-[#A18A68] fill-[#A18A68]" />
              ))}
            </div>
          ...
          <div className="flex items-center gap-4 pt-8 border-t border-gray-50">
            ...
            <div className="flex flex-col">
              <span className="text-sm font-bold text-black uppercase tracking-wide">Mariana S.</span>
  ...
  return (
    <div className="bg-white p-8 rounded-[2rem] h-full flex flex-col justify-between shadow-sm transition-all duration-700 ease-out hover:shadow-xl hover:-translate-y-1 hover:border-gray-200 border border-gray-100/50 cursor-default group/card min-h-[200px]">
      <div className="flex justify-between items-start mb-4">
        <span className="text-xs font-bold tracking-widest text-gray-300 group-hover/card:text-black transition-colors duration-500">{number}</span>
        <div className="p-2 bg-gray-50 rounded-full group-hover/card:bg-black transition-colors duration-500">
             <Move className="w-3 h-3 text-gray-400 group-hover/card:text-white transition-colors duration-500" strokeWidth={2} />
        </div>
      </div>
      
      <div>
        <h3 className="text-xl font-serif font-medium text-black mb-3 group-hover/card:translate-x-1 transition-transform duration-500">{title}</h3>
